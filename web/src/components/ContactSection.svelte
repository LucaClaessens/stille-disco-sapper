...
<div class="w-full h-full md:h-5/6 bg-gray-100 flex flex-col md:flex-row">
  <div class="relative flex-1 md:flex-3 md:p-24 h-2/3 md:h-auto">
    <h2
      class="hidden font-semibold md:block absolute top-12 left-12 text-6xl text-gray-700 z-50"
    ><FadeInBottom>{title}</FadeInBottom></h2>
...
<!-- <TwoColumnLayout>
  <span slot="left">
    <Block>
      <span slot="title">
        <BlockTitle usePadding text={title} />
      </span>
      <div slot="content" class="md:px-6 md:max-w-lg">
        <Image {...image} classes="w-full h-96" />
      </div>
    </Block>
  </span>
  <span slot="right">
